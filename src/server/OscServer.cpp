...
bool OscServer::start() {
    ...
    lo_server_thread_add_method(serverThread_, "/retro/loop/scramble_on", "iiddi",
...
void OscServer::pushStateTo(lo_address addr) {
    ...
    for (int i = 0; i < engine_.maxLoops(); ++i) {
        ...
        if (!pending) continue;
        std::string desc = opTypeDescription(pending->opType);
        if ((pending->opType == OpType::UndoLayer || pending->opType == OpType::RedoLayer)
            && pending->undoCount > 1) {
            desc += " x" + std::to_string(pending->undoCount);
        }
        lo_send(addr, "/retro/state/pending_op", "iis",
                i, quantizeToInt(pending->quantize), desc.c_str());
    }
